...
#define MAX_POINT_SIZE 150
#define MAX_POINT_AXYS 4

#define MAX_BATCH_SIZE (MAX_POINT_AXYS * MAX_POINT_SIZE) 
// The recommended size is at least 2 x batch size.
#define WRITE_BUFFER_SIZE (2 * MAX_BATCH_SIZE)
const unsigned long imu_semp_period = 50; // in ms
    ...
    if (!client.isBufferFull()) {
        ...
        if ( point2Send > 0) {
            if ( now_ms > nextPoint ) {
                ...
                sensorImu.addField("accel.x", data.accel.x);
                sensorImu.addField("accel.y", data.accel.y);
                sensorImu.addField("accel.z", data.accel.z);
